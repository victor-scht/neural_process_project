...
# ==================================================
# Required external functions (to provide in R)
# ==================================================
# simuHawkesExpoM(param, M, Tend, xi)
# intensM(s, param, times)
# hawkes_to_isjumpN(times, grid) -> list(isjumpN=..., all_spikes=...)
# generate_basis_functions(K, seed, m) -> list(bfunc=..., sigfunc=..., afunc=..., basis_params=...)
# simu_jumpdiff(X0, grid, bfunc, sigfunc, afunc, isjumpN)
...
collecestimcoeff <- function(X, U, q1, q2, Nn) {
  ...
  for (k in seq_len(Nn)) {
    ...
    if (is.null(alpha_hat)) {
      collecP <- collecP[seq_len(k - 1)]
      colleccoeffalpha <- colleccoeffalpha[seq_len(k - 1), , drop = FALSE]
      break
    }
...
main <- function() {
  ...
  # --------------------------------------------------
  intensity <- t(sapply(grid, function(s) intensM(s, param, times)))
  if (is.null(dim(intensity))) {
    intensity <- matrix(intensity, ncol = M)
  }
  ...
  truncphi <- sapply(dX / (seuil * Delta^beta_trunc), phifunc)
  ...
  penaltyb_vals <- sapply(seq_len(Nn), function(k) penaltyb(k, n, Delta, rho, sigma02))
