...
def plot_X(ts, X_true, X_hat, path):
    plt.figure(figsize=(10, 4))
    plt.plot(ts, X_true, label="X(t) true")
    plt.plot(ts, X_hat, label="X(t) reconstructed (estimated)", alpha=0.8)
    plt.xlabel("time")
    plt.ylabel("X")
...
def plot_functions(drift_true_fn, sigma_true_fn, drift_theta, sigma_phi, xgrid, out_prefix):
    plt.figure(figsize=(6, 4))
    plt.plot(xgrid, drift_true_fn(xgrid), label="f (drift) true")
    plt.plot(xgrid, drift_model(xgrid, drift_theta), label="f (drift) estimated", alpha=0.85)
    plt.xlabel("x")
    plt.ylabel("drift")
    ...
    plt.figure(figsize=(6, 4))
    plt.plot(xgrid, sigma_true_fn(xgrid), label="g (sigma) true")
    plt.plot(xgrid, sigma_model(xgrid, sigma_phi), label="g (sigma) estimated", alpha=0.85)
    plt.xlabel("x")
    plt.ylabel("sigma")
